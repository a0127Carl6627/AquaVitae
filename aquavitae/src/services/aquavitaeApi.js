...
export async function fetchDashboard() {
  ...
  const plantas = (data.plantas || []).map((p) => ({
    ...
    riesgo:
      p.nivelRiesgo?.toLowerCase() === 'alto'
        ? 'alta'
        : p.nivelRiesgo?.toLowerCase() ?? 'bajo',

    riesgoLabel: p.nivelRiesgo
      ? p.nivelRiesgo.charAt(0).toUpperCase() +
        p.nivelRiesgo.slice(1).toLowerCase()
      : '—',
...
export async function fetchKpis(plantaId) {
  ...
  const res = await fetch(
    `${BASE}/api/simulacion/kpis?plantaId=${pid}`
  );
...
export async function fetchProyeccion(plantaId, dias = 90) {
  const params = new URLSearchParams({
    dias: String(dias),
    plantaId: String(plantaId ?? 1),
  });

  const res = await fetch(
    `${BASE}/api/simulacion/proyeccion?${params}`
  );

  if (!res.ok) {
    throw new Error(`Proyección error ${res.status}`);
  }
  ...
  const chartData = (d.puntos || []).map((p, i) => ({
    ...
    bandaSup:
      d.bandaSuperior?.[i] != null
        ? parseFloat(
            d.bandaSuperior[i].valor.toFixed(1)
          )
        : null,

    bandaInf:
      d.bandaInferior?.[i] != null
        ? parseFloat(
            d.bandaInferior[i].valor.toFixed(1)
          )
        : null,
  }));
  ...
  return {
    ...
    peakValue: parseFloat(
      (d.peakValue ?? 0).toFixed(1)
    ),
  };
}

export async function fetchRecuperacion(
  plantaId,
  dias = 90
) {
  const params = new URLSearchParams({
  ...
  const res = await fetch(
    `${BASE}/api/simulacion/recuperacion?${params}`
  );

  if (!res.ok) {
    throw new Error(`Recuperación error ${res.status}`);
  }
  ...
  return (d.conIntervencion || []).map((p, i) => ({
    ...
    sinIntervencion:
      d.sinIntervencion?.[i] != null
        ? parseFloat(
            d.sinIntervencion[i].valor.toFixed(1)
          )
        : null,
  }));
}
...
export async function fetchAlerta(plantaId) {
  const res = await fetch(
    `${BASE}/api/alternativas/alerta?plantaId=${plantaId}`
  );

  if (!res.ok) {
    throw new Error(`Alerta error ${res.status}`);
  }
  ...
  return {
    ...
    estresActual: Math.round(
      (data.indiceActual ?? 0) * 100
    ),

    costoAperturaStr: formatMXN(
      data.costoApertura
    ),

    costoOperacionStr: formatMXN(
      data.costoOperacionEstimada
    ),
...
export async function fetchAlternativas(plantaId) {
  const res = await fetch(
    `${BASE}/api/alternativas/ubicaciones?plantaId=${plantaId}`
  );

  if (!res.ok) {
    throw new Error(`Alternativas error ${res.status}`);
  }
...
export async function fetchFactores(plantaId) {
  const res = await fetch(
    `${BASE}/api/alternativas/factores?plantaId=${plantaId}`
  );

  if (!res.ok) {
    throw new Error(`Factores error ${res.status}`);
  }
...
export async function fetchResumen() {
  ...
  if (!res.ok) {
    throw new Error(
      'Error al obtener resumen de usuarios'
    );
  }
...
export async function fetchUsuarios(
  ...
  const res = await fetch(
    `${BASE}/api/usuarios?page=${page}&size=${size}`,
    {
      headers: authHeaders(),
    }
  );
...
export async function fetchRolConPermisos(
  rolId
) {
  const res = await fetch(
    `${BASE}/api/usuarios/roles/${rolId}`,
    {
      headers: authHeaders(),
    }
  );
...
export async function generarContrasena() {
  const res = await fetch(
    `${BASE}/api/usuarios/generar-contrasena`,
    {
      headers: authHeaders(),
    }
  );
  ...
  if (!res.ok) {
    throw new Error(
      'Error al generar contraseña temporal'
    );
  }
...
export async function crearUsuario(dto) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      error.mensaje || 'Error al crear usuario'
    );
  }
...
export async function eliminarUsuario(id) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      error.mensaje || 'Error al eliminar usuario'
    );
  }
}
...
export async function editarUsuario(id, dto) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      error.mensaje || 'Error al editar usuario'
    );
  }
